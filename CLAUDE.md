...
This file is always-on context for every Claude Code session in this project.
It defines the mandatory workflow for any prompt that changes the codebase
(code, tests, config, docs, deps — anything committed). Purely
conversational / read-only prompts are exempt.

> Source of truth: `.agents/rules/constitution.md` — keep in sync if either is updated.
...
Applies to issues/PRs in repos linked to a GitHub Projects (v2) board.

- **Repo:** `Sinishaw/event-calendar-v3` (`https://github.com/Sinishaw/event-calendar-v3.git`)
- **Project board:** Flutter - Event Calendar V3, Project #2 (`PVT_kwHOAZCkW84BatrB`)
  — `gh project` commands should use `--owner Sinishaw --number 2`
