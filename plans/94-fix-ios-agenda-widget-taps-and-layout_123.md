...
Tapping the widget opened its URL (`eventcalendarwidget://…`), which iOS handed to Flutter's route deep linking → `pushNamed("/?d=…&t=…")` → crash ("Could not find a generator for route"). Under the app's `FlutterSceneDelegate`, home_widget's app-delegate URL hook does **not** fire, so its `widgetClicked` never delivered the tap — the route channel is the only one that receives the URL on iOS. So instead of disabling deep linking, we **handle the route ourselves**:
- **Widget URLs are path-based** (`eventcalendarwidget://open/day?d=…&t=…`, `…/open/add`) so the action survives (Flutter drops the host; the first path segment carries `day`/`add`).
- **`main.dart` `MaterialApp`**: `onGenerateRoute` opens the target for warm taps (`_widgetPageRoute`); `onGenerateInitialRoutes` boots the splash and stashes the link (`HomeWidgetService.pendingWidgetUri`) for cold start; `onUnknownRoute` returns splash (no crash). `_ContainerPageState` consumes the pending link post-frame; `_handleWidgetUri` accepts host- **or** path-based URIs (Android still uses host via `widgetClicked`).
